...
public class KdTree<T> : IEnumerable<T>, IEnumerable where T : Component
{
    ...
    {
        KdTree<T> list = new KdTree<T>(_just2D);
    ...
    {
        List<KdNode> list = new List<KdNode>(_getNodes());
        list.RemoveAt(i);
        Clear();
    ...
    private void _add(KdNode newNode)
    {
        ...
        newNode.right = null;
        newNode.level = 0;
        KdNode parent = _findParent(newNode.component.transform.position);
        ...
        float splitParent = _getSplitValue(parent);
        float splitNew = _getSplitValue(parent.level, newNode.component.transform.position);
    ...
    private KdNode _findParent(Vector3 position)
    {
        ...
        while (current != null)
        {
            float splitCurrent = _getSplitValue(current);
            float splitSearch = _getSplitValue(current.level, position);
    ...
    protected T _findClosest(Vector3 position, List<T> traversed = null)
    {
        ...
        while (openCur < _open.Length && _open[openCur] != null)
        {
            ...
            float nodeDist = _distance(position, current.component.transform.position);
